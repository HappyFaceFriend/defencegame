...
{
    Road, Wall, Empty, NoTower, NoPlayer, OutOfBound
}
...
public class GridManager : MonoBehaviour
{
    ...
    {
        return IsInBound(WorldPosToGridIndex(position));
    }
    public bool IsInBound(Vector2Int gridIndex)
    {
        return gridIndex.x >= 0 && gridIndex.x < topRight.x - bottomLeft.x && gridIndex.y >= 0 && gridIndex.y < topRight.y - bottomLeft.y;
    }
    ...
    public void SetBackgroundAt(Vector3 worldPosition, GridBackground type)
    {
        ...
        SetBackgroundAt(index, type);
    }
    public void SetBackgroundAt(Vector2Int gridIndex, GridBackground type)
    {
        backgroundGrid[gridIndex.y, gridIndex.x] = type;
    }
